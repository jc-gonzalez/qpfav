...
/*
    ...
    QWidget * existingWdg = ui->tabwdgViewers->findChild<QWidget*>(tabName);
    if (existingWdg != 0) {
        ui->tabwdgViewers->setCurrentIndex(ui->tabwdgViewers->indexOf(existingWdg));
        return;
    }
    ...
    int tabIdx = ui->tabwdgViewers->addTab(editor, tabName);
    ui->tabwdgViewers->setTabIcon(tabIdx, QIcon(iconRscName));
    ui->lstwdgNav->addItem(tabName);

    ui->tabwdgViewers->setTabsClosable(true);
    for (int i = 0; i < 4; ++i) {
        ui->tabwdgViewers->tabBar()->tabButton(i, QTabBar::RightSide)->hide();
        ui->tabwdgViewers->tabBar()->tabButton(i, QTabBar::RightSide)->resize(0, 0);
    }
    QWidget * tabbtn = ui->tabwdgViewers->tabBar()->tabButton(tabIdx, QTabBar::RightSide);
